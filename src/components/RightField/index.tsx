...
const RightField = () => {
  ...
  return (
    <Styled.RightFieldContainer>
      {viewCard ? <Card id={card.id} title={card.title} description={card.description} handleViewCard={handleViewCard} /> : (<><Styled.HeaderText>
        Доверь свою судьбу случайности!
      </Styled.HeaderText>
      <Styled.NumberContainer>
        {loading ? <Loader color="#ffffff" /> : <Styled.Number>{randomNumber}</Styled.Number>}
      </Styled.NumberContainer>
      {randomNumber ? 
      <Styled.Button onClick={handleViewCard}>Открыть карту</Styled.Button> : 
